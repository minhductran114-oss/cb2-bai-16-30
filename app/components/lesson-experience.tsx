...
export function LessonExperience({ lesson, section, store, navigate }: { lesson: LessonModule; section: SectionId; store: CourseStore; navigate: (hash: string) => void }) {
  ...
  return <main className={`app lesson-${lesson.id}`}>
    ...
    <aside className="sidebar">
      <div className="lesson-switch" role="group" aria-label="Chọn bài học">{([23, 24] as const).map((id) => <button key={id} className={lesson.id === id ? "lesson-chip active" : "lesson-chip"} onClick={() => navigate(`#/bai/${id}/overview`)}><span>Bài</span><strong>{id}</strong></button>)}</div>
    ...
    <section className="workspace">
      ...
      <footer className="section-footer"><button className={completed.includes(completionKey) ? "complete-button completed" : "complete-button"} onClick={() => store.toggleSection(completionKey)}>{completed.includes(completionKey) ? "✓ Đã hoàn thành phần này" : "Đánh dấu đã hoàn thành"}</button><span>Nội dung Bài {lesson.id} được giữ từ PWA mẫu và nay tham gia vào lộ trình, kho kiến thức, lịch ôn và sổ lỗi sai chung.</span></footer>
    </section>
